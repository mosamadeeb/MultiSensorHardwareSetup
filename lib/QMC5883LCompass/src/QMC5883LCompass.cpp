...
QMC5883LCompass::QMC5883LCompass(TwoWire *wireObj): wireObj(wireObj) {
}
...
void QMC5883LCompass::_writeReg(byte r, byte v){
	wireObj->beginTransmission(_ADDR);
	wireObj->write(r);
	wireObj->write(v);
...
void QMC5883LCompass::read(){
	wireObj->beginTransmission(_ADDR);
	wireObj->write(0x00);
	int err = wireObj->endTransmission();
	if (!err) {
		wireObj->requestFrom(_ADDR, (byte)6);
		_vRaw[0] = (int)(int16_t)(wireObj->read() | wireObj->read() << 8);
		_vRaw[1] = (int)(int16_t)(wireObj->read() | wireObj->read() << 8);
		_vRaw[2] = (int)(int16_t)(wireObj->read() | wireObj->read() << 8);
		...
		//byte overflow = wireObj->read() & 0x02;
		//return overflow << 2;
	}
}
